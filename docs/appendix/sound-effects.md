...
Stonescript can play game sound effects using the [`play` command](/reference/commands#play-sound-pitch).

Most sounds have variations that play randomly or sequentially. If the same sound is played more than once in a single frame it is ignored. If more than 5 sounds are played per second they are throttled.

## Example

```stonescript
// Play 'unequip' whenever the player loses health
var lasthp = hp
?hp < lasthp
  play unequip
lasthp = hp
